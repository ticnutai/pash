...
export function OmerReminderPopup({ reminder, onDismiss }: OmerReminderPopupProps) {
  ...
  return (
    <Dialog open={!!reminder} onOpenChange={(open) => !open && onDismiss()}>
      <DialogContent className="sm:max-w-[420px] text-center p-0 overflow-hidden" dir="rtl">
        ...
        <div className="px-6 pb-6 space-y-5">
          ...
          <div className="space-y-2">
            <p className="text-xl font-bold leading-relaxed">{reminder.message}</p>
            <p className="text-sm text-muted-foreground font-medium">{reminder.label}</p>
          </div>
